...
export function YouTubeVideoInbox() {
  ...
  async function addSource(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (sourceLocation === "all") {
      setMessage({ type: "error", text: "Use a specific place for source location (for example Cebu, Quiapo, or Davao). ‘All’ is reserved for the app filter." });
      return;
    }
      ...
      <section className="card">
        ...
        <form className="flex flex-col gap-5" onSubmit={(event) => void addSource(event)}>
          <div className="form-grid">
            ...
            <label className="field"><span>Default source location <small className="text-muted">optional</small></span><input name="default_video_source_location" className="input" placeholder="Cebu" maxLength={48} /><small className="text-muted">Use a specific place, not “All”.</small></label>
          ...
          {sources.map((source) => <article key={source.id} className="rounded-xl border border-line bg-white p-4">
            ...
            {editingSource?.id === source.id && <form className="mt-4 border-t border-line pt-4" onSubmit={(event) => void updateSource(event, source)}>
              <div className="form-grid">
                ...
                <label className="field"><span>Default source location</span><input name="default_video_source_location" className="input" defaultValue={source.default_video_source_location ?? ""} placeholder="Cebu" maxLength={48} /><small className="text-muted">Use a specific place, not “All”.</small></label>
